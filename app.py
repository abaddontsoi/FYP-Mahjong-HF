...
YOLO_26_DETECTOR_PATH   = "MJ_tile_detection_yolo26_fine_tune.pt"                # detection .pt
...
detector26   = YOLO(YOLO_26_DETECTOR_PATH)      # task='detect'
detector26.zero_grad()
...
def pipeline26(input_img: np.ndarray): 
    """
    Gradio will pass an RGB numpy array (H, W, 3).
    Return the annotated RGB array.
    """
...
def pipeline(input_img: np.ndarray):
    ...
    annotated_img26, detected_tiles26 = pipeline26(input_img)
    return annotated_img11, ", ".join(detected_tiles11), annotated_img26, ", ".join(detected_tiles26)
...
demo = gr.Interface(
    ...
    outputs=[
        ...
        gr.Image(type="numpy", label="Annotated Detection + Classification on YOLOv26"),
        gr.TextArea(label="Detected tiles"),
        ],
